  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Webhook Captures</h1>
          <p className="text-base text-slate-500 dark:text-slate-400 mt-2">Monitor and inspect incoming webhook requests.</p>
        </div>
        <button
      ...
      {/* Create Form */}
      {showCreateForm && (
        <form onSubmit={handleCreateEndpoint} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/60 dark:border-slate-700 shadow-sm p-6 mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base font-semibold text-slate-700 dark:text-slate-300">Create Webhook Endpoint</h3>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="w-full sm:w-auto sm:flex-1 px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 text-base text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 transition-all"
            />
            <button
              type="submit"
              disabled={creating}
        ...
        <div className="mb-8">
          ...
          <div className="space-y-2">
            {endpoints.map((ep) => (
              <div
                key={ep.id}
                className="flex flex-col sm:flex-row sm:items-center justify-between bg-white dark:bg-slate-900 rounded-xl border border-slate-200/60 dark:border-slate-700 shadow-sm px-4 sm:px-5 py-3.5 group hover:border-slate-300/80 dark:hover:border-slate-600 transition-all gap-2"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="p-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 shrink-0">
                    <svg className="w-3.5 h-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                  </div>
                  <div className="min-w-0">
                    <span className="font-medium text-base text-slate-700 dark:text-slate-300">{ep.name || ep.short_id}</span>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-xl border border-slate-200/60 dark:border-slate-700 shadow-sm">
          ...
          <p className="text-slate-600 dark:text-slate-300 font-medium text-xl">No captures yet</p>

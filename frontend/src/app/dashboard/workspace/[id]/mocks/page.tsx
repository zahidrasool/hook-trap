  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Mock Endpoints</h1>
          <p className="text-base text-slate-500 dark:text-slate-400 mt-2">Create and manage mock API responses</p>
        </div>
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <button
            onClick={() => setShowYamlImport(true)}
            className="inline-flex items-center gap-2 px-5 py-2.5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg font-medium text-base transition-all duration-200 shadow-sm"
          >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 border border-slate-200 dark:border-slate-700 rounded-lg text-base bg-white dark:bg-slate-900 shadow-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors duration-200 outline-none placeholder:text-slate-300 dark:placeholder:text-slate-500 dark:text-white"
          />
        </div>
        <div className="relative">
          <select
            ...
            className="appearance-none px-4 pr-9 py-3 border border-slate-200 dark:border-slate-700 rounded-lg text-base bg-white dark:bg-slate-900 shadow-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors duration-200 outline-none font-medium text-slate-600 dark:text-slate-300 cursor-pointer"
          >
            {HTTP_METHODS.map((m) => (
      ...
      {filteredMocks.length === 0 ? (
        <div className="text-center py-20">
          {mocks.length === 0 ? (
            <>
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-400 mb-5">
              ...
              <p className="text-xl font-semibold text-slate-700 dark:text-slate-300 mb-2">No mock endpoints yet</p>
            ...
            <>
              ...
              <p className="text-base text-slate-500 dark:text-slate-400">No mocks match your filters.</p>
            </>
          )}
        </div>
      ) : (
      ...
      >
        <div className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">Name (optional)</label>
            <input
              type="text"
              value={newName}
              onChange={(e) => setNewName(e.target.value)}
              placeholder="e.g., List Users"
              className="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-lg text-base bg-white dark:bg-slate-900 shadow-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors duration-200 outline-none placeholder:text-slate-300 dark:placeholder:text-slate-500 dark:text-white"
            />
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">Method</label>
              <div className="relative">
                <select
                  value={newMethod}
                  onChange={(e) => setNewMethod(e.target.value)}
                  className="w-full appearance-none px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-lg text-base bg-white dark:bg-slate-900 shadow-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors duration-200 outline-none font-semibold cursor-pointer dark:text-white"
                >
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">Path</label>
              <input
                type="text"
                value={newPath}
                onChange={(e) => setNewPath(e.target.value)}
                placeholder="/api/users"
                className="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-lg text-base font-mono bg-white dark:bg-slate-900 shadow-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors duration-200 outline-none placeholder:text-slate-300 dark:placeholder:text-slate-500 dark:text-white"
              />
            </div>
          </div>
